...
import '../showcase_starsystem/lib/main.dart';
import '../showcase_starsystem/lib/embedded_assets.dart';
...
void main() {
  ...
  group('StarSystemGame Integration Test', () {
    ...
    test('Full integration of components and logic', () async {
      ...
      // Tick the dispatcher
      ui.PlatformDispatcher.instance.onBeginFrame?.call(const Duration(milliseconds: 16));
      ui.PlatformDispatcher.instance.onDrawFrame?.call();

      // Test ui clicks
      final box = game.scene.getCaste<UIBoundingBox>('UIBoundingBox').get(game.spawnPlanetBtnId);
      box?.pointerId = 1.0;
      ui.PlatformDispatcher.instance.onBeginFrame?.call(const Duration(milliseconds: 32));

      box?.pointerId = -1.0;
      ui.PlatformDispatcher.instance.onBeginFrame?.call(const Duration(milliseconds: 48));

      final asteroidBox = game.scene.getCaste<UIBoundingBox>('UIBoundingBox').get(game.spawnAsteroidBtnId);
      asteroidBox?.pointerId = 1.0;
      ui.PlatformDispatcher.instance.onBeginFrame?.call(const Duration(milliseconds: 64));
      asteroidBox?.pointerId = -1.0;
      ui.PlatformDispatcher.instance.onBeginFrame?.call(const Duration(milliseconds: 80));

      final cometBox = game.scene.getCaste<UIBoundingBox>('UIBoundingBox').get(game.spawnCometBtnId);
      cometBox?.pointerId = 1.0;
      ui.PlatformDispatcher.instance.onBeginFrame?.call(const Duration(milliseconds: 96));
      cometBox?.pointerId = -1.0;
      ui.PlatformDispatcher.instance.onBeginFrame?.call(const Duration(milliseconds: 112));
