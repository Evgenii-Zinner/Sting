...
void main() async {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    int blue = 200 + (i * 15);
    fillRect(playerImg, x1: i * 32, y1: 0, x2: i * 32 + 31, y2: 31, color: ColorRgb8(0, 0, blue));
    // Draw a small lighter square in the middle to show direction/animation
    fillRect(playerImg, x1: i * 32 + 8 + i, y1: 8 + i, x2: i * 32 + 23 - i, y2: 23 - i, color: ColorRgb8(100, 100, 255));
  }
  ...
  for (int i = 0; i < 4; i++) {
    ...
    int red = 200 + (i * 15);
    fillRect(enemyImg, x1: i * 32, y1: 0, x2: i * 32 + 31, y2: 31, color: ColorRgb8(red, 0, 0));
    // Draw eyes or pattern
    fillRect(enemyImg, x1: i * 32 + 8, y1: 8, x2: i * 32 + 12, y2: 12, color: ColorRgb8(255, 255, 0));
    fillRect(enemyImg, x1: i * 32 + 20, y1: 8, x2: i * 32 + 24, y2: 12, color: ColorRgb8(255, 255, 0));
  }
  ...
  fillCircle(projectileImg, x: 8, y: 8, radius: 8, color: ColorRgb8(255, 255, 0));
  fillCircle(projectileImg, x: 8, y: 8, radius: 4, color: ColorRgb8(255, 128, 0));
  File('${outDir.path}/projectile.png').writeAsBytesSync(encodePng(projectileImg));
  ...
  for (int y = 0; y < 4; y++) {
    for (int x = 0; x < 4; x++) {
      ...
      fillRect(tilemapImg, x1: x * 32, y1: y * 32, x2: x * 32 + 31, y2: y * 32 + 31, color: c);
      // Draw grid outline for tile separation visually
      drawLine(tilemapImg, x1: x * 32, y1: y * 32, x2: x * 32 + 31, y2: y * 32, color: ColorRgb8(0, 0, 0));
      drawLine(tilemapImg, x1: x * 32, y1: y * 32, x2: x * 32, y2: y * 32 + 31, color: ColorRgb8(0, 0, 0));
    }
  }
